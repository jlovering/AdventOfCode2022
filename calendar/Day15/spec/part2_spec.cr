...
describe ds.dayShortName do
    describe "Part2", tags: "part2" do
        ...
        it "part2 tim solution", tags: "solution" do
            p2 = Part2.new(ds.dayPath.to_s + "/input/tim_input.txt")
            result = p2.run(4000000)
            Spec.after_suite {
                puts "\nPart 2 Tim result: #{result}"
            }
            if result != "" && ds.enableSubmit
                aoC_result = aoCc.submit_solution(ProblemPart::B, result)
                aoCc.flush
